...
export default function ContatosV2Page() {
  ...
  const [historicoMensagens, setHistoricoMensagens] = useState([
    ...
    { id: 9, data: '20/07/2021 12:49', mensagem: 'Seja ben-vindo(a) a DPE/RS! Guarde este numero...', remetente: 'Marta Wayne', defensoria: 'UNIDADE DE SISTEMAS DE INFORMAÇÃO', contato: '(11) 97805-7049', status: 'pendente', acaoRespondida: true, numRespostas: 1 },
  ...
  useEffect(() => {
    ...
    if (novasConversas.length > 0) {
      // Adiciona as novas conversas no início da lista
 ...
 const renderAcaoIcon = (item) => {
  ...
  // Função para filtrar apenas a conversa ativa atual (exclui conversas encerradas anteriormente)
  const filterActiveConversation = (messages) => {
    if (!messages || messages.length === 0) return [];
    
    // Encontra o índice da última mensagem de encerramento
    let lastEndIndex = -1;
    ...
    if (lastEndIndex >= 0) {
      ...
      // Se há mensagens após o encerramento, mostra apenas essas
      if (messagesAfterEnd.length > 0) {
        return messagesAfterEnd;
      }
      
      // Se não há mensagens após o encerramento, mostra toda a conversa (incluindo o encerramento)
      // Isso evita o histórico ficar em branco
      return messages;
    }
    
    // Se não houve encerramento, retorna todas as mensagens
    return messages;
  };
  ...
  const handleOpenHistoryDetailModal = (item) => {
    ...
    if (item.chatTranscript && item.chatTranscript.length > 0) {
        ...
        if (activeChat && activeChat.messages && activeChat.messages.length > 0) {
            // Usa o histórico real do chat global e filtra conversa ativa
            const mappedMessages = activeChat.messages.map(msg => ({
                id: msg.id,
                sender: msg.sender, // Mantém o sender original (defensor/assistido)
            ...
            chatToShow = filterActiveConversation(mappedMessages);
             ...
             // Histórico mockado realista para demonstração
             ...
             chatToShow = [
               ...
               },
               {
                 ...
                 sender: 'assistido',
                 name: item.remetente,
                 text: 'Muito obrigado doutor(a)! 🙏\n\nFico no aguardo. Tenha um ótimo dia!',
    ...
    // Se não há conversa ativa para mostrar, pode mostrar mensagem explicativa
  ...
  const handleContinueConversation = (item) => {
    ...
    if (wasEnded) {
      ...
      setApprovalChatHistory([]); // Inicia com histórico vazio
    } else {
      // Se não foi encerrada, continua a conversa existente
      ...
      >
          ...
          <Group justify="flex-start" mt="md">
              ...
              {(() => {
                ...
                // Botão aparece se há transcript E a conversa foi encerrada
